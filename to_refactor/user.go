...
//
//	err := DatabaseClient.QueryRow(fmt.Sprintf("SELECT * FROM users WHERE username = '%s' ;", d)).Scan(&user.Username, &user.Email, &user.Password, &user.Description, &user.ID)
//	if err != nil {
...
//
//	err := DatabaseClient.QueryRow(sqlQuery).Scan(&passwordCheck)
...
//		err = DatabaseClient.QueryRow(sqlQuery).Scan(&user.Username, &user.Email, &user.Password, &user.Description, &user.ID)
//		return ""
//	} else {
...
//
//	var scanStruct User
//	row := DatabaseClient.QueryRow(fmt.Sprintf("SELECT * FROM users WHERE email = '%s' LIMIT 1;", user.Email))
...
//
//	//Checking of existing users with this email
//	row = DatabaseClient.QueryRow(fmt.Sprintf("SELECT * FROM users WHERE username = '%s' LIMIT 1;", user.Username))
//	e = row.Scan(&scanStruct.Username, &scanStruct.Email, &scanStruct.Password, &scanStruct.Description, &scanStruct.ID)
//	if e != nil && e != sql.ErrNoRows {
//		return "error"
//	}
